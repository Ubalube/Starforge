...
public class ChunkGeneratorSpore implements IChunkGenerator
{
		...
		public ChunkGeneratorSpore(World worldIn, long seed) {
		...
		public Chunk generateChunk(int x, int z) {
			...
			for (int i = 0; i < abyte.length; ++i) {
				abyte[i] = (byte) Biome.getIdForBiome(BiomeInit.SPORE);
			}
		...
		private void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, Biome[] biomesIn) {
			...
			for (int i = 0; i < 16; ++i) {
				for (int j = 0; j < 16; ++j) {
					...
					generateBiomeTerrain(this.world, this.rand, primer, x * 16 + i, z * 16 + j, this.field_186002_u[j + i * 16], BiomeInit.SPORE);
				}
			}
		}
